...
*/
function probeSupport(activeMappings) {
...
*/
function addMapping(intPort, extPort, lifetime, activeMappings,
	controlUrl) {
	...
	function _handleControlUrl(controlUrl) {
		return new Promise((F, R) => {
			...
			utils.getPrivateIps().then((privateIps) => {
...
*/
function deleteMapping(extPort, activeMappings, controlUrl) {
	// Do the UPnP flow to delete a mapping, and if successful, remove it from
	// activeMappings and return true 
	return sendDeletePortMapping(controlUrl, extPort).then(() => {
 ...
 */
function getUpnpControlUrl() {
	return _getUpnpControlUrl().catch((err) => { /* what? */ });
};
...
*/
function fetchControlUrl(ssdpResponse) {
	...
	var _fetchControlUrl = new Promise((F, R) => {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState === 4) {
				...
				if (!xmlDoc.includes("WANIPConnection") || startIndex === 12) {
...
*/
function sendAddPortMapping(controlUrl, privateIp, intPort, extPort, lifetime) {
...
* @return {string} The response string to the AddPortMapping request
*/
function sendDeletePortMapping(controlUrl, extPort) {
	// Promise to send an AddPortMapping request to the control URL of the router
	var _sendDeletePortMapping = new Promise((F, R) => {
