...
function probeSupport(activeMappings, routerIpCache) {
	const probePort = utils.NAT_PMP_PROBE_PORT;
...
*/
function addMapping(intPort, extPort, lifetime,
						  activeMappings, routerIpCache) {
	...
	function _sendPmpRequestsInWaves() {
		return utils.getPrivateIps().then((privateIps) => {
			...
			return _sendPmpRequests(matchedRouterIps).then((mapping) => {
...
var deleteMapping = function(extPort, activeMappings, routerIpCache) {
	...
	function _sendDeletionRequestsInWaves() {
		return utils.getPrivateIps().then((privateIps) => {
			...
			var matchedRouterIps = utils.arrAdd(routerIpCache,
												utils.filterRouterIps(privateIps));
			var otherRouterIps = utils.arrDiff(utils.ROUTER_IPS, matchedRouterIps);
	...
	return _sendDeletionRequestsInWaves().then(_deleteFromActiveMappings)
...
function sendPmpRequest(routerIp, intPort, extPort, lifetime) {
	...
	return Promise.race([
		utils.countdownReject(2000, 'No NAT-PMP response', () => {
			utils.closeSocket(socket);
		}),
		_sendPmpRequest
	]);
